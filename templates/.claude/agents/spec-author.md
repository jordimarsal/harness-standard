...
Your only job is to produce three files for **exactly one** `pending` feature from `feature_list.json`:
...
2. Take the lowest `id` `pending` feature from `feature_list.json`. Create `specs/<name>/` if it doesn't exist.
...
6. Change the feature status to `spec_ready` in `feature_list.json`.
