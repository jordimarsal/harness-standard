...
1. Run `./init.sh` and verify it finishes without errors. If it fails, **stop** and fix the environment before touching code.
2. Read `progress/current.md` to understand where the last session left off.
3. Read `feature_list.json`. ALL features follow **Spec Driven Development** — see §4 of this file.
...
|-----------------------------|-----------------------------------------------------------------------------|----------------------------------|
| `feature_list.json`         | Feature list with status (`pending` / `spec_ready` / `in_progress` / `done` / `blocked`) | Always, at start                 |
...
| `specs/<feature>/`          | `requirements.md` + `design.md` + `tasks.md` (Kiro-style)                  | Before implementing any feature  |
...
| `CHECKPOINTS.md`            | Objective criteria for "correct final state"                                | For self-assessment              |
| `.claude/agents/`           | Subagent definitions (`leader`, `spec-author`, `implementer`, `reviewer`)   | If you orchestrate work          |
...
- **Do not mark a task `done` without green tests.** Run `./init.sh` and ensure the test block passes 100%.
...
2. The leader dispatches `spec-author`, who creates `specs/<name>/{requirements,design,tasks}.md` and marks status as `spec_ready`.
3. **Pause.** The human reads the spec at `specs/<name>/` and approves (or requests changes).
...
When `"parallel": true` in `feature_list.json` project config:
...
- If a tool doesn't behave as expected, **do not invent a workaround**: document the block in `progress/current.md` and stop the session.
